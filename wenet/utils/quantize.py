# https://github.com/666DZY666/micronet/blob/master/micronet/compression/quantization/wqaq/dorefa/quantize.py
...
class ActivationQuantizer(nn.Module):
    ...
    # 量化/反量化
    def forward(self, input):
        delta = torch.max(torch.abs(input)) / math.pow(2.0, self.a_bits-1)
        bound = math.pow(2.0, self.a_bits-1)
        min_val = - bound
        max_val = bound - 1
...
class WeightQuantizer(nn.Module):
    ...
    # 量化/反量化
    def forward(self, input):
        delta = torch.max(torch.abs(input)) / math.pow(2.0, self.w_bits-1)
        bound = math.pow(2.0, self.w_bits-1)
        min_val = - bound
        max_val = bound - 1
        rounded = torch.floor(input / delta + 0.5)
        clipped_value = torch.clamp(rounded, min_val, max_val) * delta
