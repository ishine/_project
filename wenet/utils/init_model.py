...
def init_model(configs):
    ...
    # quantize
    wbit = configs['encoder_quant_conf'].get('wbit', 32)
    abit = configs['encoder_quant_conf'].get('abit', 32)
    if wbit != 32 or abit != 32:
        from wenet.utils.quantize import prepare
        encoder = prepare(encoder, a_bits=int(abit), w_bits=int(wbit), quant_inference=True)
